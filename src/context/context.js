...
const GithubProvider = ({ children }) => {
  ...
  const searchGithubUser = async (user) => {
    ...
    if (response) {
      ...
      // fetching repos and followers by Promise.allSettled() for ui updation at once instead of one by one
  ...
  const checkRequests = () => {
    axios
      ...
      .then(({ data }) => {
        ...
        if (remaining === 0) {
          toggleError(true, "Sorry! You have exceeded the hourly rate limit.");
        }
      })
      .catch((err) => console.log(err));
  };
